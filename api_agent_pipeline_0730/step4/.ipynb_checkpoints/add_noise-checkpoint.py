...
def process_res(input_):
    res = []
    for api in input_.split('；'):
        temp = {}
        if not re.search(r'名称：(.*?)，工具名',api):
            return
        else:
            temp['api_name'] = str(re.search(r'名称：(.*?)，工具名',api).group(1))
        if re.search(r'工具名：(.*?)，所需参数',api):
            temp['tool_name'] = str(re.search(r'工具名：(.*?)，所需参数',api).group(1))
        else:
            return
        res.append(f"{temp['tool_name']}_{temp['api_name']}")
    return res
...
for line in open('train_standard_name', 'w'):
    ...
    for item in query_dict[key]:
        if len(item) <= 10:
            ...
            if key in step4_result_dict:
                ...
                if is_in(step4_result_dict[key], item):
                    cnt_1 += 1
                    if cnt_1 <= ratio_1:
                        print(json.dumps(dic, ensure_ascii=False))
                elif match and is_in(step4_result_dict[key], item):
                    if len(step4_result_dict[key]) == len(item) and str(step4_result_dict[key]) != str(item):
                        cnt_2 += 1
                        if cnt_2 <= ratio_2:
                            print(json.dumps(dic, ensure_ascii=False))
                    elif len(step4_result_dict[key]) == len(item) and str(step4_result_dict[key]):
                        res = ""
                        random.shuffle(item)
                        for api_tag in item:
                            res += "{" + api_dict[api_tag] + "}"
